...
//				}
    ...
    public void createFileForEach(String keySize)throws ParseException
    {
...
//            cl.append("Runtime.getRuntime().addShutdownHook(new Thread(() -> {");
//                cl.append("System.out.println(\"Application is shutting down...EWRTYFTY\");");
//            cl.append("}));");
            ...
            //Keys
            try{
                ProcessBuilder pb = new ProcessBuilder("bash", "./scripts/set_keys.sh", keySize);
                //ProcessBuilder pb = new ProcessBuilder("bash", "/home/vagrant/fs/set_keys.sh", keySize);
                pb.inheritIO();
                Process p = pb.start();
                ...
                System.out.println("set_keys.sh exited with code: " + exitCode);
            }catch(Exception ex) {
                ex.printStackTrace();
            }
...
//			cl.append("\r\nProcessBuilder pb = new ProcessBuilder(\"bash\", \"/home/vagrant/fs/setup.sh\");");
            cl.append("\r\npb.inheritIO();");
            cl.append("\r\nProcess p = pb.start();");
            cl.append("\r\nBufferedReader errorReader = new BufferedReader(new InputStreamReader(p.getErrorStream()));");
            cl.append("\r\nString line = null;");
            ...
            //might need make logs directory
            //cl.append("\r\npw = new PrintWriter(new FileWriter(\"./logs/output_"+name+".txt\", true));\r\n");
...
//			+"\r\n						}"
    ...
    {StringBuilder as1 = new StringBuilder();
        as1.append("package larva;"
                ...
                +"\r\n}}}}");
        try{
            PrintWriter pw1 = new PrintWriter(Compiler.outputDir+"/larva/LogWriter.java");
            pw1.write(as1.toString());
            pw1.close();
        }catch(Exception ex)
        {
    ...
    public void createFiles(String keySize)throws ParseException
    {
        if (id == 0)//root
        {
        ...
        this.createFileForEach(keySize);
    ...
    //SSS
    public String toJava(String keySize) throws ParseException
    {
        createFiles(keySize);
        return "Files READY!!!";
    }
...
//    }
    ...
    public static String redirectOutput(String stats) {
        return stats.replaceAll(
                "System \\.out \\.println \\((.*?)\\);",
                "if (acceptingLogs.get()) { \nif (! queue.offer($1)) {\n    System.out.println(\"SealFS writing busy, log missed: \" + $1);\n}}"
        );
    }
